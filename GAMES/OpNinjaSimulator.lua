...
if v:IsA("Tool") and string.find(v.Name, "Katana") then
return v
end
end
end
...
task.spawn(function()
       game:GetService("RunService").RenderStepped:Connect(function(dt)
              ...
              if dt > 0 and remote2 then
                     if autoUpgrade then
                            if game:GetService("Players").LocalPlayer.PlayerGui.MainGui.UpgradeF.SwordF.SwordImgBtn.ImageColor3 ~= Color3.fromRGB(0, 20, 0) and (abc() - Time) >= 0.25 then
                                   Time = abc()
                                   Replay(upgrade1)
                            end
                            if game:GetService("Players").LocalPlayer.PlayerGui.MainGui.UpgradeF.ClassF.ClassImgBtn.ImageColor3 ~= Color3.fromRGB(0, 0, 105) and (abc() - Time) >= 0.25 then
                                   Time = abc()
                                   Replay(upgrade2)
                            end
                            if game:GetService("Players").LocalPlayer.PlayerGui.MainGui.UpgradeF.AscendF.AscendImgBtn.ImageColor3 ~= Color3.fromRGB(0, 0, 105) and (abc() - Time) >= 0.25 then
                                   Time = abc()
                                   Replay(upgrade3)
                            end
                     end
